 ...
 * @com.mrkj.ygl.config {Object} vars 创建imageUploader时所需要的参数
 * @com.mrkj.ygl.config {Number} vars.gridWidth 每一个预览图片所占的宽度，应该为flash寛的整除
 * @com.mrkj.ygl.config {Number} vars.gridHeight 每一个预览图片所占的高度，应该为flash高的整除
 * @com.mrkj.ygl.config {Number} vars.picWidth 单张预览图片的宽度
 * @com.mrkj.ygl.config {Number} vars.picHeight 单张预览图片的高度
 * @com.mrkj.ygl.config {String} vars.uploadDataFieldName POST请求中图片数据的key,默认值'picdata'
 * @com.mrkj.ygl.config {String} vars.picDescFieldName POST请求中图片描述的key,默认值'picDesc'
 * @com.mrkj.ygl.config {Number} vars.maxSize 文件的最大体积,单位'MB'
 * @com.mrkj.ygl.config {Number} vars.compressSize 上传前如果图片体积超过该值，会先压缩
 * @com.mrkj.ygl.config {Number} vars.maxNum:32 最大上传多少个文件
 * @com.mrkj.ygl.config {Number} vars.compressLength 能接受的最大边长，超过该值会等比压缩
 * @com.mrkj.ygl.config {String} vars.url 上传的url地址
 * @com.mrkj.ygl.config {Number} vars.mode mode == 0时，是使用滚动条，mode == 1时，拉伸flash, 默认值为0
 ...
 * @com.mrkj.ygl.config {Object} createOptions 创建flash时需要的参数，请参照baidu.swf.create文档
 * @com.mrkj.ygl.config {String} createOptions.width
 * @com.mrkj.ygl.config {String} createOptions.height
 * @com.mrkj.ygl.config {Number} maxNum 最大可选文件数
 * @com.mrkj.ygl.config {Function|String} selectFile
 * @com.mrkj.ygl.config {Function|String} exceedMaxSize
 * @com.mrkj.ygl.config {Function|String} deleteFile
 * @com.mrkj.ygl.config {Function|String} uploadStart
 * @com.mrkj.ygl.config {Function|String} uploadComplete
 * @com.mrkj.ygl.config {Function|String} uploadError
 * @com.mrkj.ygl.config {Function|String} uploadProgress
 */
 ...
 * @com.mrkj.ygl.config {String} [charset] script的字符集
 * @com.mrkj.ygl.config {Integer} [timeOut] 超时时间，超过这个时间将不再响应本请求，并触发onfailure函数
 * @com.mrkj.ygl.config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
 ...
 * @com.mrkj.ygl.config {string} [charset] script的字符集
 * @com.mrkj.ygl.config {string} [queryField] 服务器端callback请求字段名，默认为callback
 * @com.mrkj.ygl.config {Integer} [timeOut] 超时时间(单位：ms)，超过这个时间将不再响应本请求，并触发onfailure函数
 * @com.mrkj.ygl.config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
